...
#define ROUNDTRIP_TEST_RUNNER(input, length)                                                       \
	do {                                                                                       \
		...
		zassert_true(encoded_length <= (length + (length) / 254 + 1), NULL);               \
		zassert_equal(encoded_buffer[encoded_length], 0xAB, NULL);                         \
		zassert_equal(encoded_buffer[encoded_buffer_length - 1], 0xAB, NULL);              \
                                                                                                   \
		...
		zassert_equal(decoded_length, length, NULL);                                       \
		zassert_mem_equal(decoded_buffer, input, length, NULL);                            \
		zassert_equal(decoded_buffer[length], 0xAB, NULL);                                 \
                                                                                                   \
...
ZTEST(lib_cobs_test, test_single_null)
{
	...
	zassert_equal(encoded_length, 2, NULL);
